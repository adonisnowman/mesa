...
class NovelController extends BaseController
{
    ...
    public function OpenAI(){
        ...
        $Message = Tools::fix_element_Key(self::$PostData,'Message');
        $curl = curl_init();    // create cURL session

        $API_KEY = "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"; // login to https://beta.openai.com/account/api-keys and create an API KEY
        ...
        $data = array( // cUrl data
            ...
            "messages" => array(["role"=> "user", "content"=> $Message]), // choose your prompt (what you ask the AI)
            "temperature" => 0.7   // max amount of tokens to use per request
        );
        curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode($data));
        
        $Return['OpenAI'] = JSON_decode(curl_exec($curl));         
        curl_close($curl);    
        return $Return;
    }
    //讀取單筆資料
